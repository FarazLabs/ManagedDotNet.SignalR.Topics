...
public sealed class EndpointOptions
{
    ...
    public EndpointOptions RequireAuthorization(params string[] policyNames)
    {
        ...
        if (policyNames is null || policyNames.Length == 0)
            throw new MisconfiguredException(
                "RequireAuthorization was called with no policy names — that would silently skip hub auth. " +
    ...
    public EndpointOptions RequireAuthorization(params IAuthorizeData[] authorizeData)
    {
        ...
        if (authorizeData is null || authorizeData.Length == 0)
            throw new MisconfiguredException(
                "RequireAuthorization was called with no authorize data — that would silently skip hub auth. " +
