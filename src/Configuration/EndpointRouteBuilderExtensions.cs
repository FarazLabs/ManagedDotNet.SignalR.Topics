...
public static class EndpointRouteBuilderExtensions
{
    ...
    )
    {
        ...
        ILogger logger = endpoints.ServiceProvider.GetService<ILoggerFactory>()
            ?.CreateLogger("ManagedDotNet.SignalR.Topics")
            ?? NullLogger.Instance;
        ...
        foreach (EndpointOptions endpoint in options.Endpoints)
        {
            ...
            // Hub RequireAuthorization + topic AllowAnonymous: topic flag cannot open the connection
            if (endpoint.RequiresAuthorization)
            {
                foreach (HandleOnServerConfiguration route in endpoint.HandleOnServerConfigurations.Values)
                {
                    if (!route.IsAnonymousAllowed)
                        continue;

                    logger.LogWarning(
                        "Hub '{HubType}' at '{Path}' requires authorization, but topic '{Topic}' uses AllowAnonymous(). " +
                        "Topic AllowAnonymous only skips topic-level auth; clients must still authenticate to connect (single shared Handle endpoint).",
                        endpoint.HubType.FullName,
                        endpoint.Path,
                        route.Topic);
                }
            }
